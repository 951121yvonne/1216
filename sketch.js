...
class Animation {
    ...
    display(x, y, displayW, displayH) {
        ...
        let drawX = x + (displayW - targetW) / 2;
        let drawY = y + (displayH - targetH) / 2; 
...
function keyPressed() {
    ...
    if (questionActive) {
        if (keyCode === 75 && currentQuestion) { 
            ...
            if (currentQuestionType === 'questioner' && currentQuestioner) {
                hintCharacter.x = currentQuestioner.x;
                hintCharacter.y = currentQuestioner.y + 10;
            } else if (currentQuestionType === 'potion' && potion) {
                hintCharacter.x = potion.x;
                hintCharacter.y = potion.y + 40;
            } else {
